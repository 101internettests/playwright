...
def test_first(page: Page):
    ...
    expect(page.locator('//h1[contains(text(), "Интернет-провайдеры")]')).to_contain_text('Интернет-провайдеры')
    ...
    expect(page.locator('// div[contains(text(), "Отзывы о 101 интернет на Яндекс Картах")]')).to_have_text('Отзывы о 101 интернет на Яндекс Картах')
    ...
    expect(page.locator('//div[contains(text(), "© 2008-2024 «101 Интернет» — поиск провайдеров по адресу")]')).to_contain_text('© 2008-2024 «101 Интернет» — поиск провайдеров по адресу')
    expect(page.locator('//div[contains(text(), "Мы в соцсетях")]')).to_contain_text('Мы в соцсетях')
    expect(page.get_by_role('link', name='https://vk.com/ru101internet')).to_be_enabled()
    expect(page.get_by_role('link', name='https://www.odnoklassniki.ru/group/51961592610882')).to_be_enabled()
